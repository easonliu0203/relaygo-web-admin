...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: referrals, error: referralsError } = await db.supabase
      ...
      .in('driver_firebase_uid', firebaseUids);
    ...
    if (referrals && referrals.length > 0) {
      const affiliateIds = referrals.map(r => r.affiliate_id);
    ...
    const result = drivers.map(driver => {
      ...
      const driverDocs = documents?.filter(d => d.driver_id === user?.firebase_uid) || [];
      const driverPhotos = vehiclePhotos?.filter(p => p.driver_id === driver.user_id) || [];
      const referral = referrals?.find(r => r.driver_firebase_uid === user?.firebase_uid);
      const affiliate = referral ? affiliates.find(a => a.id === referral.affiliate_id) : null;
